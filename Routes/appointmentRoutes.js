const express = require("express");
const Router = express.Router(); //type of this is function
const appointmentController = require("../http/Controllers/appointmentController");
const transactionMiddleware = require("../http/middlewares/transactionMiddleware");
const authMiddleware = require("../http/middlewares/authMiddleware");

Router.post("/create", transactionMiddleware(appointmentController().create));
Router.get("/timeslots", appointmentController().getTimeSlots);
Router.get("/timeslots/dashboard", authMiddleware, appointmentController().getTimeSlots);
Router.get("/countries", appointmentController().getCountries);

Router.post("/disable-date-and-time-slots", authMiddleware, appointmentController().disableDateAndTimeSlots);
Router.get("/get-disabled-date-and-time-slots/dashboard", authMiddleware, appointmentController().getDisabledDateAndTimeSlots);
Router.get("/get-disabled-date-and-time-slots", appointmentController().getDisabledDateAndTimeSlots);

Router.delete("/delete-disabled-date-and-time-slots/:id", authMiddleware, appointmentController().deleteDisabledDateAndTimeSlots);




module.exports = Router;